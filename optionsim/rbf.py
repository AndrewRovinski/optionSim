...
class RadialBasisInterpolation:
    ...
    def __init__(self, X, f, degree=0, epsilon=1, smooth=0,
        ...
        radial_dist = scipy.spatial.distance.cdist(X, X)

        self.smooth = max(
            float(smooth),
            1e-10 * scipy.linalg.norm(f) / np.sqrt(len(f)))  # Scale by ||f||

        K = self._kernel(radial_dist) + np.eye(self.X.shape[0]) * self.smooth
        P = RadialBasisInterpolation.vandermond(X, degree=self.degree)

        # Build the matrix
        Z = np.zeros([P.shape[1]] * 2)
        z = np.zeros(P.shape[1])
        KP = np.block([[K, P], [P.T, Z]])
        b = np.hstack([f, z])
        coef = scipy.linalg.solve(KP, b)
        self.rbf_coef = coef[:self.X.shape[0]]
        self.poly_coef = coef[self.X.shape[0]:]
    ...
    def _kernel(self, r):
        ...
        elif not isinstance(self.kernel, (str, str)):
            raise ValueError(
                'Kernel must be a callable with signature (r,epsilon),'
                'an integer (spline) or a valid string')
    ...
    def vandermond(X, degree):
        ...
        *total* order degree
        ...
        index = np.asarray(
            RadialBasisInterpolation.total_index(degree, ndim),
            dtype=int)
